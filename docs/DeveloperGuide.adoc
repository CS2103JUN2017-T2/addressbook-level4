...
|`* * *` |new user |check the glossary |get help with commands

|`* * *` |user |add a new entry |

|`* * *` |user |view an entry |check its details

|`* * *` |user |edit an entry |change its particulars (e.g. time, deadline)
...
|`* * *` |user |search entries by name |retrieve specific entries more quickly

|`* * *` |user |perform a search using keywords |find specific entries

|`* * *` |user |store my entries in a folder managed by syncing softwares |ensure all my devices have the same list
...
|`* * *` |user |automatically insert floating entries into free time-slots |remember to do tasks without deadlines

|`* * *` |user |view the next n entries |to see upcoming events to attend/tasks to finish

|`* * *` |user that attends events |add entries that have fixed start and end times |

|`* * *` |user |adjust the start and end date of entries |update tasks when requirements change

|`* *` |user |add a priority level to entries |filter them according to importance
...
|`* *` |user |search entries by added date |
...
|`* *` |advanced user |undo my commands any number of times |to revert undesirable changes
...
|`* *` |user |search for free days in a time period (e.g. next 1 month) |use free days to slot tasks

|`* *` |user |launch and minimize the application with a keyboard shortcut |open directly (from Desktop) without using the mouse

|`* *` |user |categorize entries (e.g. work,shopping,etc) |

|`* *` |user |tick off sub-entries within an entry as completed |to track my progress

|`* *` |user |organize my list |

|`* *` |user |swap entries with a single command |

|`* *` |user |identify priority of tasks based on colours in the GUI |view optimally for faster recognition
...
|`* *` |user |clear all entries existing before a certain date |keep calendar organized

|`* *` |user |configure defaults (e.g. storage path) |

|`* *` |user |sort entries by tags/deadline/priority |plan which tasks to do first

|`* *` |user |clear archive/deleted entries folder |prevent the UI from being cluttered

|`* *` |user |list floating tasks (without deadlines) |finish these in my free time

|`* *` |user |mark applications without deadlines as not important by default |

|`* *` |user |delete duplicate entries |keep my list organized
...
(For all use cases below, `ProductNameHere` refers to the *System* and `user` refers to the *Actor*, unless specified otherwise.)
...
2. ProductNameHere creates the entry and moves it to active list. <br>
Use case ends.
...
i) ProductNameHere shows an error message and prints the proper usage syntax and example command for adding new entries.
...
2. ProductNameHere searches for the entry.
3. ProductNameHere makes the modification and moves the entry to archive. <br>
Use case ends.

*Extensions*

2a. ProductNameHere finds more than one entry with the same keywords in the names.

i) ProductNameHere displays an indexed list of all such entries and asks for an index from the user.
ii) User specifies index.
iii) ProductNameHere checks if index is valid, if not, ProductNameHere shows an error message and returns to step 2a ii.
iv) Use case resumes from step 3.
...
2. ProductNameHere searches for the entry.
3. ProductNameHere makes the modification and moves the entry to deleted bin. <br>
Use case ends.

*Extensions*

2a. ProductNameHere finds more than one entries with the same keywords in the names.

i) ProductNameHere displays an indexed list of all such entries and ask for an index from the user.
ii) User specifies index.
iii) ProductNameHere checks if index is valid, if not, ProductNameHere shows an error message and returns to step 2a ii.
iv) Use case resumes from step 3.
...
1. User requests to list entries with a duration argument without a start date, and ending at the end of the same day.
2. ProductNameHere shows the list of entries sorted by end date/time, starting from the soonest. <br>
Use case ends.
...
1. User requests to list entries with a duration starting Monday next week, and ending Sunday next week.
2. ProductNameHere shows the list of entries sorted by end date/time, starting from the soonest. <br>
Use case ends.
...
1. User requests to list entries with a duration starting Monday next week, and ending Sunday next week.
2. ProductNameHere shows the list of entries sorted by end date/time, starting from the soonest. Use case ends.
...
2. ProductNameHere adds the new entry.
...
4. ProductNameHere finds the relevant entry.
5. ProductNameHere makes the modification. <br>
Use case ends.

*Extensions*

4a. ProductNameHere finds more than one entries with the same keywords in the names.

i) ProductNameHere displays an indexed list of all such entries and ask for an index from the user.
ii) User specifies index.
iii) Use case resumes from step 5.
...
2. ProductNameHere obediently executes the command.
3. User undo changes.
4. ProductNameHere removes all changes made by the last command. <br>
Use case ends.
...
1. User uses the `restore` command to recover a deleted entry by providing keywords that match the entry name.
2. ProductNameHere finds the relevant entry.
3. ProductNameHere restores the entry. <br>
Use case ends.

*Extensions*

2a. ProductNameHere finds more than one deleted or archived entries with the same keyword in the names.

i) ProductNameHere displays an indexed list of all such entries and ask for an index from the user.
ii) User specifies index.
iii) Use case resumes from step 3.
...
.  The product should be a desktop app.
.  The Command Line Inteface should be the primary mode of input.
.  The software should work stand-alone and not be a plug-in to another app.
.  The software should use text files for storage and not relational databases.
.  The data should be stored locally in an editable text file.
.  The code should follow the Object-oriented paradigm.
.  The product should work without requiring an installer.
.  The product should favor DOS style commands over Unix-style commands.
...
Entry
____
Any item stored in the system (e.g. events, tasks, note to self). +
An entry *must* have:
