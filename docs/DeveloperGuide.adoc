...
The class diagram above shows how the Logic component is structured. `Logic` applies the Facade pattern, and the API calls are made through the `LogicManager` class to the subsequent Parser classes.
...
** The parser classes make use of utility methods in `ParserUtil`, `ArgumentMultimap`, etc.
...
image::PowerMatchClassDiagram.jpg[width="800"]
