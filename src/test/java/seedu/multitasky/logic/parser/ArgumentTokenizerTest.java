...
public class ArgumentTokenizerTest {
    ...
    private final Prefix slashTag = new Prefix("/tag");

    @Test
    public void tokenize_emptyArgsString_noValues() {
        String argsString = "  ";
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(argsString, slashTag);

        assertPreambleEmpty(argMultimap);
        assertArgumentAbsent(argMultimap, slashTag);
    }

    private void assertPreamblePresent(ArgumentMultimap argMultimap, String expectedPreamble) {
        assertEquals(expectedPreamble, argMultimap.getPreamble().get());
    }

    private void assertPreambleEmpty(ArgumentMultimap argMultimap) {
        assertTrue(argMultimap.getPreamble().get().isEmpty());
    }
    ...
    public void tokenize_oneArgument() {
        // Preamble present
        String argsString = "  Some preamble string /tag Argument value ";
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(argsString, slashTag);
        assertPreamblePresent(argMultimap, "Some preamble string");
        assertArgumentPresent(argMultimap, slashTag, "Argument value");

        // No preamble
        argsString = " /tag   Argument value ";
        argMultimap = ArgumentTokenizer.tokenize(argsString, slashTag);
        assertPreambleEmpty(argMultimap);
        assertArgumentPresent(argMultimap, slashTag, "Argument value");
    ...
    public void tokenize_multipleArgumentsWithRepeats() {
        ...
        String argsString = "SomePreambleString -t dashT-Value ^Q ^Q -t another dashT value /tag slashTag value -t";
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(argsString, slashTag, dashT, hatQ);
        assertPreamblePresent(argMultimap, "SomePreambleString");
        assertArgumentPresent(argMultimap, slashTag, "slashTag value");
    ...
    public void tokenize_multipleArgumentsJoined() {
        String argsString = "SomePreambleString/tag slashTag joined-tjoined -t not joined^Qjoined";
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(argsString, slashTag, dashT, hatQ);
        assertPreamblePresent(argMultimap, "SomePreambleString/tag slashTag joined-tjoined");
        assertArgumentAbsent(argMultimap, slashTag);
