...
public class EntryBook implements ReadOnlyEntryBook {
	...
	/**
	 * Adds an entry to the entry book. Also checks the new entry's tags and
	 * updates {@link #tags} with any new tags found, and updates the Tag
	 * objects in the entry to point to those in {@link #tags}.
	 */
	 ...
	 * {@code editedReadOnlyEntry}. {@code EntryBook}'s tag list will be updated
	 * with the tags of {@code editedReadOnlyEntry}.
	 * 
	 ...
	 */
	public void updateEntry(ReadOnlyEntry target, ReadOnlyEntry editedReadOnlyEntry) throws EntryNotFoundException {
		...
		// not tagged to any entry
		// in the entry list.
		entries.updateEntry(target, editedEntry);
	}

	/**
	 * Ensures that every tag in this entry: - exists in the master list
	...
	private void syncMasterTagListWith(Entry entry) {
		...
		// Rebuild the list of entry tags to point to the relevant tags in the
		// master tag list.
	...
	/**
	 * Ensures that every tag in these entries: - exists in the master list
	 * {@link #tags} - points to a Tag object in the master list
	 * 
	 * @see #syncMasterTagListWith(Entry)
	 */
